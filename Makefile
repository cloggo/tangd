...
DOCKER_BUILDER_CMD := docker run --rm --user root -it -v $(HOME)/.npmrc:/root/.npmrc -w /app -v $(CURDIR)/tmp:/root -v $(CURDIR)/app:/app
DOCKER_SERVICE_CMD := docker run --rm -it --user root -v $(HOME)/.npmrc:/root/.npmrc -w /app -v $(CURDIR)/app:/app
